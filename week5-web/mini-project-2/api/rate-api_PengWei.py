...
def upsert_client_rate(client_id, rate):
    ...
    import requests
    param={'client_id': 'rate'}
    response = requests.post("http://127.0.0.1:5000/rate", json=param)  # what to post?
...
def test_upsert_rate():
    upsert_client_rate('client4', 0.4)
    upsert_client_rate("client5", 0.5)
    assert get_rate('client4') == 0.4
    assert get_rate('client5') == 0.5
# -- TODO END: Part 6
